#!/usr/bin/env bash
function _install_git_config() {
    running "installing gitconfig template to ~/.gitconfig"
    cp -f ./homedir/.gitconfig ~/.gitconfig
    ok
}
...
function install_essential_dev_tools() {
    ...
    if [[ "$CURRENTSHELL" != "$HOMEBREW_ZSH" ]]; then
        bot "setting newer homebrew zsh ($HOMEBREW_ZSH) as your shell (password required)"
        sudo dscl . -change /Users/$USER UserShell $SHELL $HOMEBREW_ZSH > /dev/null 2>&1
        ok
    fi
...
# ###########################################################
function enable_spyware_ad_blocking_by_hosts() {
...
function configure_git() {
    running "checking SSH Key..."
    if [ -f "~/.ssh/id_rsa" ]; then
        ok
    else
        action "generating an SSH Key"
        ssh-keygen -t rsa -b 4096 -f "/Users/$(whoami)/.ssh/id_rsa" -C "$email" -N ''
        ok
    fi

    action "copying your SSH key to your clipboard"
    pbcopy < ~/.ssh/id_rsa.pub
    ok
    sleep 1

    action "add the generated SSH key to your GitHub account. It has been copied to your clipboard"
    sleep 2
